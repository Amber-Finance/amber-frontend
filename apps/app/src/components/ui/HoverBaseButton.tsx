...
export const HoverBaseButton = React.forwardRef<HTMLButtonElement, HoverBaseButtonProps>(
  ({ children, className, dotColor = 'bg-primary', ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          'group relative w-auto cursor-pointer overflow-hidden rounded-full border bg-background p-2 px-6 text-center font-bold',
          className,
        )}
        {...props}
      >
