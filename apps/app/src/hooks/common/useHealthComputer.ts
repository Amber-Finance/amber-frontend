...
import { buildHealthComputer } from '@/utils/blockchain/healthComputer'
import {
  BorrowTarget,
  compute_health_js,
...
export default function useHealthComputer(updatedPositions?: Positions) {
  ...
  useEffect(() => {
    ...
    const hasPositions =
      healthComputer.positions.lends.length > 0 || healthComputer.positions.debts.length > 0
    ...
    try {
      ...
      if (healthValues.liquidation_health_factor === null) {
        ...
        setHealthFactor(Math.min(Number(healthValues.liquidation_health_factor), 10))
      }
    } catch {
      setHealthFactor(0)
    }
  ...
  const computeMaxBorrowAmount = useCallback(
    (denom: string, target: BorrowTarget) => {
      ...
      try {
        return new BigNumber(max_borrow_estimate_js(healthComputer, denom, target)).integerValue()
      } catch {
        return BN_ZERO
      }
    },
