...
export function useMarketMetrics(market: Market): MarketMetrics {
  return useMemo(() => {
    ...
    const depositCapAmount = new BigNumber(depositCap).shiftedBy(-market.asset.decimals).toNumber()

    const depositCapUsagePercent =
      depositCapUsd > 0
        ? Math.min(100, parseFloat(((reserveSizeUsd / depositCapUsd) * 100).toFixed(2)))
        : 0
    ...
    const supplyApy = convertAprToApy(parseFloat(market.metrics.liquidity_rate))
    ...
    const liquidationThreshold = parseFloat(market.params.liquidation_threshold) * 100
    const liquidationPenalty = parseFloat(market.params.liquidation_bonus.starting_lb) * 100
